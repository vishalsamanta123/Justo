...
import { BLACK_COLOR, DATE_TIME_FORMAT, GREEN_COLOR, PURPLE_COLOR, ROLE_IDS, WHITE_COLOR, YELLOW_COLOR } from '../../../../components/utilities/constant';
...
const BookingListItem = (props: any) => {
    ...
    const { view } = usePermission({
        view: props?.type === 'readyToBook' ? 'view_ready_to_book' :
            props?.type === 'register' ? 'view_ready_to_book' : 'view_booking'
    })
    console.log("🚀 ~ file: BookingListItem.tsx:31 ~ props.items:", props.items)
        ...
        <View style={styles.IteamView}>
            <View style={styles.Txtview} >
                <View style={styles.projectContainer}>
                    <Text style={styles.projectTxt}>
                        {props?.type === 'register' ? "Registration Date :" :
                            "Booking Date :"}
                    </Text>
                </View>
            ...
            <View style={[styles.Txtview, { borderBottomWidth: 0 }]} >
                ...
                <View style={styles.nameContainer}>
                    ...
                    <Text style={[styles.nameTxt, {
                        color:
                            props?.items?.booking_status === 1 || props?.items?.booking_status === 4 ? 'red' :
                                props?.items?.booking_status === 2 || props?.items?.booking_status === 3 ? GREEN_COLOR :
                                    BLACK_COLOR
                    }]}>{
                            props.type === 'register' ?
                                "Registered" :
                                props?.items?.booking_status === 1 ? 'Booking Pending' :
                                    props?.items?.booking_status === 2 ? 'Booking Confirm' :
                                        props?.items?.booking_status === 3 ? 'Registered' :
