import { View, Text, ScrollView, TouchableOpacity, Linking, Image, FlatList } from 'react-native'
import React from 'react'
import styles from './styles'
import Button from '../../../../components/Button'
import { normalize, normalizeWidth } from '../../../../components/scaleFontSize'
import strings from '../../../../components/utilities/Localization'
import { BLACK_COLOR, DATE_BY_DAY, DATE_TIME_FORMAT, Isios, ROLE_IDS } from 'app/components/utilities/constant'
                    ...
                    <View style={styles.nameContainer}>
                        ...
                        <Text style={[styles.nameTxt, {
                            ...
                            item?.booking_status === 1 ? 'Pending' :
                                item?.booking_status === 2 ? 'Booking Confirm' :
                    ...
                    <View style={styles.Txtview}>
                        <View style={styles.projectContainer}>
                            <Text style={styles.projectTxt}>{
                                props.type === 'cancel' ? "Cancel Booking By" :
                                    "Booking By"}</Text>
                        </View>
                        <View><Text>:</Text></View>
                        <View style={styles.nameContainer}>
                            <Text style={styles.nameTxt}>{
                                props.type === 'cancel' ?
                                    item?.cancel_by_name ?
                                        item?.cancel_by_name : strings.notfount
                                    :
                                    item?.booking_by_name ?
                                        item?.booking_by_name : strings.notfount
                            }</Text>
                        </View>
                    </View>
                    <View style={styles.Txtview}>
                            ...
                            <Text style={styles.nameTxt}>{
                                props.type === 'cancel' ?
                                    item?.canceldate === '' ||
                                        ...
                                        strings.notfount : moment.utc(item?.canceldate).format(DATE_TIME_FORMAT)
                                    :
                                    item?.booking_date === '' ||
                                        item?.booking_date === null ||
                                        item?.booking_date === undefined ?
                                        strings.notfount : moment.utc(item?.booking_date).format(DATE_TIME_FORMAT)
                            }</Text>
                        </View>
                    </View>
                    <View style={styles.Txtview}>
                        ...
                        <View style={styles.nameContainer}>
                            <Text style={styles.nameTxt}>
                                {item?.bookings?.booking_by?.user_name ?
                                    item?.bookings?.booking_by?.user_name : null}</Text>
                        </View>
                    </View>
                    <View style={styles.Txtview}>
                        <View style={styles.projectContainer}>
                                                ...
                                                return (
                                                    <View style={styles.docsVw}>
                                                        {itm?.document_type === "document" ? (
                                                            <TouchableOpacity onPress={() => props.OpenDoc(`${item?.register_document_base_url}${itm?.document}`)}>
                                                                <Image
                                                            ...
                                                            <>
                                                                {itm?.document_type === "image" ?
                                                                    <TouchableOpacity onPress={() => props.OpenDoc(`${item?.register_document_base_url + "/" + itm?.document}`)}>
                                                                        <Image
                                                                            source={{ uri: item?.register_document_base_url + "/" + itm?.document }}
                                                                            style={styles.imageVw}
                                                                            resizeMode={'contain'}
                                                                        />
                                                                    </TouchableOpacity>
